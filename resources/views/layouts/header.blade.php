...
<ul class="c-header-nav ml-auto mr-4">
    ...
    <li class="c-header-nav-item mr-3">
        <a class="btn btn-primary btn-pill {{ request()->routeIs('app.pos.index') ? 'disabled' : '' }}" href="{{ route('app.pos.index') }}">
            <i class="bi bi-cart mr-1"></i> Punto de Venta
        </a>
    </li>
    @endcan
    ...
    <li class="c-header-nav-item dropdown d-md-down-none mr-2">
        ...
        <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg pt-0">
            <div class="dropdown-header bg-light">
                <strong>{{ $low_quantity_products->count() }} Notificaciones</strong>
            </div>
            @forelse($low_quantity_products as $product)
                <a class="dropdown-item" href="{{ route('products.show', $product->id) }}">
                    <i class="bi bi-hash mr-1 text-primary"></i> Productos: "{{ $product->product_code }}" hay poca cantidad!
                </a>
            @empty
                <a class="dropdown-item" href="#">
                    <i class="bi bi-app-indicator mr-2 text-danger"></i> No hay notificaciones disponibles.
                </a>
            @endforelse
        </div>
    </li>
